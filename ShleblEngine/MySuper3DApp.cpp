...
#pragma region Window init
	...
	auto screenWidth = 500;
	auto screenHeight = 500;
...
#pragma endregion Window init
	...
	while (!isExitRequested) {
		...
		float redValue = (std::sin(2 * pi() * (totalTime + 0.0f)) + 1) / 2;
		float blueValue = (std::sin(2 * pi() * (totalTime + 1.0f / 3.0f)) + 1) / 2;
		float greenValue = (std::sin(2 * pi() * (totalTime + 2.0f / 3.0f)) + 1) / 2;
